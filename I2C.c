...
#define BA_SDA_PIN      12//5       // SDA signal pin
#define BA_SCL_PIN      13//8       // SCL signal pin
...
void write_byte(uint8_t address, uint8_t subAddress, uint8_t data)
{
    ...
    ret_code_t err_code;
    m_xfer_done = false;
    err_code = nrf_drv_twi_tx(&i2c, address, &temp[0], 2, false);
    APP_ERROR_CHECK(err_code);
